...
58. Investigate and suppress repeated nested-executor model refresh noise (`failed to refresh available models: timeout waiting for child process to exit`) so successful `supervisor` runs do not accumulate misleading runtime-error tails in stderr.
59. Prevent ordinary spec-refinement runs from syncing oversized canonical diffs when post-run atomicity validation fails; a run that expands a node past the acceptance ceiling should be redirected into `split_required` follow-up without preserving an overgrown canonical rewrite by default.
60. Add regression coverage for the case where executor stdout says `RUN_OUTCOME: done` but validator-enforced atomicity converts the run into `split_required`, so final canonical state, loop accounting, and review gating all reflect the validator outcome rather than the raw executor claim.
