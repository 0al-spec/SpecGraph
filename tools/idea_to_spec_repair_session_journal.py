...
def _source_artifact(
    *,
    ...
    missing_stage_keys = missing_stages or set()
    if key in missing_stage_keys:
...
def _validate_artifact(
    *,
    ...
    missing_stage_keys = missing_stages or set()
    if artifact.get("missing_optional_stage") is True and key not in missing_stage_keys:
        findings.append(
            _finding(
                finding_id=f"{key}_missing_optional_stage_untrusted",
                severity="review_required",
                message=(
                    f"{key} declares missing_optional_stage in content. Missing optional "
                    "stages are trusted only when established by the journal loader from "
...
def _source_ref_mismatch_findings(
    *,
    ...
    for artifact_key, field_path, expected_key in checks:
        if artifact_key in missing_stage_keys:
            continue
        if expected_key in missing_stage_keys:
            continue
...
def build_idea_to_spec_repair_session_journal(
    *,
    ...
    for key, artifact in artifacts.items():
        findings.extend(
            _validate_artifact(key=key, artifact=artifact, missing_stages=missing_stage_keys)
        )
    findings.extend(
    ...
    if "clarification_answers" in missing_stage_keys:
        accepted_answers = []
    else:
        accepted_answers = _accepted_answers(clarification_answers)
    if "ontology_decisions" in missing_stage_keys:
        decisions = []
    else:
