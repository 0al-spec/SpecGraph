...
- Prefer the repository Makefile shortcuts for routine supervisor/viewer/test operations instead of direct verbose commands: `make viewer-surfaces`, `make dashboard`, `make backlog`, `make proposal-spec-trace`, `make metrics-source-promotion`, `make metric-packs`, `make metric-pack-drift`, `make metric-pack-adapters`, `make metric-pack-runs`, `make metric-pricing`, `make conversation-memory`, `make conversation-memory-map`, `make conversation-memory-pressure`, `make implementation-work`, `make review-feedback`, `make test`, and `make test-supervisor`. Use direct `python3 tools/supervisor.py ... --output-mode full` only when a task explicitly needs the complete artifact on stdout.
