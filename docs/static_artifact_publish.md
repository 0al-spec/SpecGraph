...
cannot provide the URL. The validated predecessor is rehydrated before
lifecycle refresh so it remains authoritative in the generated workspace
manifest. Empty manual inputs and ordinary push/PR builds do not apply an
overlay.
...
Every product workspace bundle also derives
`runs/product_workspace_decisions.json` from canonical
`apiVersion: specgraph.io/v0alpha1`, `kind: Node`,
`metadata.type: decision` records below `specs/`. The versioned
